...
exports.types = {
    ...
    string: {
        write: (s, data, path) => {
            ...
            exports.types.Buffer.write(Buffer.from(s), data, path);
        },
        read: (state) => {
            return exports.types.Buffer.read(state).toString();
        }
    },
    Buffer: {
        write: (B, data, path) => {
            ...
            exports.types.uint.write(B.length, data, path);
            data.appendBuffer(B);
        },
        read: (state) => {
            const length = exports.types.uint.read(state);
    ...
    bigint: {
        write: (BN, data, path) => {
            ...
            exports.types.Buffer.write(Buffer.from(BN.toString(16), 'hex'), data, path);
        },
        read: (state) => {
            return BigInt('0x' + exports.types.Buffer.read(state).toString('hex'));
        }
    },
    boolean: {
        write: (b, data, path) => {
    ...
    json: {
        write: (j, data, path) => {
            exports.types.string.write(JSON.stringify(j), data, path);
        },
        read: (state) => {
            return JSON.parse(exports.types.string.read(state));
        }
    },
    oid: {
        write: (o, data, path) => {
    ...
    regex: {
        ...
        read: (state) => {
            const source = exports.string.read(state), flags = state.readUInt8(), g = flags & 0x1 ? 'g' : '', i = flags & 0x2 ? 'i' : '', m = flags & 0x4 ? 'm' : '';
